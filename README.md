...
Who is this for?
- anyone who would like to set up a quick artists portfolio.

What is inside?
- a premodelled system of categories and artworks that fits the needs of an artist who is creating artworks in series or categories, but still needs a single URL for every each piece and possibility to add good looking notes and comments to the works.
- a pre0built systemt o create any page from a few basic component blocks
...
# Build and CICD
is set up so far only for the Frontend, as the strapi might change its definitions of models while used on the remote, so it could get a bit messy or lead to data loss if CICD was oushing builds there.
